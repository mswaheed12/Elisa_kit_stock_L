...
def send_scheduled_email(subject, html_body):
    # Read properties injected securely out from GitHub Secrets variables vault
    ...
    if not sender or not password or not raw_receivers:
        print("Error: Missing secure mail server settings configuration variables.")
        return
    ...
    server.quit()
    print("Scheduled email reports successfully transmitted.")
...
if run_mode == "sunday":
    # Build complete layout summary report format
    ...
    <p>Current operational overview breakdown status totals:</p>
    <ul>
    ...
    if not critical_rows.empty:
        html += "<h4>🚨 Critical Action Required:</h4><table border='1' cellpadding='5'><tr><th>Product</th><th>Lot</th><th>Runs Left</th></tr>"
        for _, r in critical_rows.iterrows():
            html += f"<tr><td>{r['Elisa Kit Name']}</td><td>{r['Lot no.']}</td><td style='color:red;'>{r[exp_col]}</td></tr>"
        html += "</table>"
    send_scheduled_email(subject, html)

elif run_mode == "scan":
    # Only email if active warnings are present
